...
manipulation in this code. Use ??package.name for each one to access help.
...
# across NAICS levels. The following code creates the column `value_interp`, 
# which duplicates `val_norm` and estimates missing values for up to 2 years of 
# consecutive missing data in each category. Starting with NAICS 3-digit:
NAICS3 <- NAICS3 %>%
...
# Repeats the same procedure for NAICS 4-digit 
NAICS4 <- NAICS4 %>%
  group_by(GEO, NAICS_Code, Principal.statistics) %>%
  arrange(REF_DATE, .by_group = TRUE) %>%
  mutate(val_interp = zoo::na.approx(val_norm, x = REF_DATE, na.rm = FALSE
...
NAICS3and4 <- NAICS3and4[ !NAICS3and4$NAICS_Code %in% c("3122", "3123"), ]

# Append calculated rows (residuals) and rename labels
NAICS3and4 <- rbindlist(list(NAICS3and4, diff_rows_NAICS3and4)
                                    , use.names = TRUE, fill = TRUE)

setorder(NAICS3and4, REF_DATE, GEO, Principal.statistics, NAICS_Code)

# Rename NAICS labels for consistency
NAICS3and4$NAICS[
  which(NAICS3and4$NAICS_Code == "3111")] <- "3111 Animal food Manufacturing"
NAICS3and4$NAICS[
  which(NAICS3and4$NAICS_Code == "3117")] <- "3117 Seafood product preparation and packaging"

```

3d. Identify imputed or interpolated data

The script also tags all imputed or interpolated cells in red font so they can 
be easily identified. Using [if/then] logic, it checks each row for missing 
values that were filled in during the previous steps. If a value was imputed or 
interpolated, the font colour becomes red.
