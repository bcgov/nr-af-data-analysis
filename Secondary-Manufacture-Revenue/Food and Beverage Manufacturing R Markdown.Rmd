...
Ensure you have the following packages installed: - **cansim:** package
that allows STC data to be pulled into the environment using API -
**data.table:** enahnced version enabling high-performance data
manipulation for datasets that take up about 1GB in RAM or higher
...
# 3. Addressing data suppression: ,  and row-level residuals
...
3b. Time series interpolation: the script aggregates 4‑digit industries
into 3‑digit totals and uses time-series interpolation only for British
Columbia where consecutive years of data are suppressed but values exist
on either side.
...
3c. Residuals are calculated by subtracting known sub‑industry totals
from their parent category, allowing the model to calculate and display
an “other” industry row (one for food, one for beverages, tobacco and
cannabis) that replaces suppressed rows and ensures that the sum of
sub-totals matches the total.
...
they can be easily identified. Using [if/then?]
...
sharepoint_path <- 
  "C:/Users/lebautis/Government of BC/SICI Data Library Subsite - Unpublished Data"
